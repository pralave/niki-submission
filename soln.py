		...
		with open("output.txt", "a") as text_file:
			...
			for line in f: ## read every textfile line by line
				...
				for item in line_list:
					item = item.replace("+91", "") ## remove +91 from the beginning of a number for example "+918290093302 = 8290093302"
					item = item.replace("-", "")
...
if __name__ == "__main__":
	...
	map(get_num, glob.glob(os.path.join(path, "test_directory/*.txt"))) ## map every file to get_num() function to get all the numbers.
